...
function write_excel(dir, sheetNames, headers, data) {
    ...
    // 导出 Excel
    XLSX.writeFile(wb, dir + '/生成设班大表.xlsx');
}

function generate_detail(dir) {
        ...
        if (err) console.log('query class: class-data is missed!');
            ...
            for (let i = 0; i < grids_list.length; i++) {
                ...
                for (let j = 0; j < merge_list.length; j++) {
                    ...
                    if (worksheet[XLSX.utils.encode_cell({r: range.s.r, c: range.s.c})] == null) continue;
...
ipc.on('open-detail-generate-dialog', function (event) {
    dialog.showOpenDialog({
        properties: ['openDirectory']
    }, function (files) {
        if (files) event.sender.send('selected-detail-generate-directory', files);
        else return;
        var dir = files[0];
        generate_detail(dir);
    })
